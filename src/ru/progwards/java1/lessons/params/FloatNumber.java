...
/*   Добрый день!
Не смотря на то, что робот не хочет принимать Задачу 1, решил отправить код на проверку потому, что мой вариант мне кажется логически ближе к истине.
Например, робот выдаёт такое сообщение:
ERROR: Тест "Метод toString()" не пройден. Метод возвращает неверное значение. Экземпляр класса создан при помощи вызова конструктора new FloatNumber(true, 146336, 3).
Возвращено значение: 1.46336E8, ожидалось: 1.46336E2

Мне кажется, мой ответ правильный, так как мантисса должна быть умножена на 10 в степени ехр. А робот фактически не умножает, а делит.
И дальше цепочкой по всем методам всё в том же роде, проблема со степенью.

P.S. Ну и ещё одна претензия там к количеству нулей после запятой в методе toString(), хотя это никак не оговаривалось и на результате не сказывается.
*/
